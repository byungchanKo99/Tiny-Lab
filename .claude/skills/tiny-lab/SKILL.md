...
The state spec may include `"engine": "codex"` to force a specific state to run on codex instead of claude. In native agent mode you are the Claude Code runner, so you cannot literally hand off to the other backend inside the same chat session.
...
- Otherwise execute the `runner_command` from `tiny-lab brief --json` yourself and warn the user once that the intended engine was `codex` but native mode is using `claude`.
