...
Codex chat** instead of spawning the engine via `tiny-lab run`.
The state machine is the same; the executor is you (this session) instead
of a `Codex -p` subprocess.
...
2. Native runner mode runs inside the active Codex/Codex chat session. It uses the same `.state.json`, same workflow JSON, same prompt renderer, same state gate policy, and same completion advancement logic.
...
The state spec may include `"engine": "codex"` to force a specific state to run on codex instead of Codex. In native agent mode you are the Codex runner, so you cannot literally hand off to the other backend inside the same chat session.

- If `tiny-lab brief` reports `engine: codex` and the user wants the intended backend, suggest dropping back to CLI mode (`tiny-lab run --engine codex`) for that state, then resuming native mode.
- Otherwise execute the `runner_command` from `tiny-lab brief --json` yourself and warn the user once that the intended engine was `codex` but native mode is using `Codex`.

When the user wants to switch back to autonomous mode mid-workflow, remind them they can leave the chat and run `tiny-lab run` to let the engine drive the rest; `.state.json` carries the position over.
